...
def setup():
    global field, punto, particulas, factor, s1, particulas2, s2
    py5.size(720, 1080)
    field = VectorialField(
        py5.width, py5.height, punto[0], punto[1], caso, invertir
    )
    s1 = py5.load_shape("DATA/Transformaciones_kawai2.svg")
    s2 = py5.load_shape("DATA/luz_lampara.svg")
    field.modificar_campo_from_svg(s2, 0, INTERIOR, invertir)
    for i in range(n):
        particulas.append(
            Particle(punto[0]+py5.random(-10, 10), punto[1]+py5.random(0, margen))
...
class Particle():
    ...
    def update(self):
        ...
        if (self.x < 0 
            ...
            self.x = punto[0]+ py5.random(-10, 10)
            self.y = punto[1]+ py5.random(0, margen)
...
if __name__ == "__main__":
    ...
    create_video(RUTA, fps, f"corgi_lampara_{caso}")
    remove_png(RUTA)