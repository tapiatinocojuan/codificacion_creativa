...
def setup():
    global field, punto, particulas, factor
    py5.size(720, 1080)
    field = VectorialField(
        py5.width, py5.height, punto[0], punto[1], caso, invertir
    )
    #field.modificar_campo(250, 250, 100, 0, INTERIOR, -1)
    #field.modificar_campo(250, 250, 150, 0, EXTERIOR, -1)
    s1, rutas = get_point_from_svg("DATA/batman.svg", 0.01, py5.width)
    field.modificar_campo_rect(
         0, 0, py5.width, py5.height/2, py5.width/2, py5.height/2, caso, INTERIOR, invertir
    )
    # field.modificar_campo_rect(
    #      py5.width/2, 0, py5.width/2, py5.height/2, 3*py5.width/4, py5.height/4, caso, INTERIOR, invertir
    # )
    field.modificar_campo_rect(
         0, py5.height/2, py5.width, py5.height/2, py5.width/2, py5.height/2, caso, INTERIOR, -invertir
    )
    # field.modificar_campo_rect(
    #      py5.width/2, py5.height/2, py5.width/2, py5.height/2, 3*py5.width/4, 3*py5.height/4, caso, INTERIOR, invertir
    # )
    field.modificar_campo_from_svg(s1, 7, INTERIOR, -invertir)
    for ruta in rutas:
        print("ejecutando x ruta")
        for i, punto in enumerate(ruta[:-1]):
            field.modificar_campo_linea(
                punto[0], punto[1], 
    ...
    for i in range(n):
        particulas.append(
            Particle(py5.random(0,py5.width), py5.random(0,py5.height))
        )
    py5.background(0)

def draw():
    global particulas, field
    #py5.background(0)
...
class Particle():
    ...
    def draw(self):
        ...
        py5.fill(r, g, 100, alpha)
        py5.stroke(r, g, 100, alpha)
...
if __name__ == "__main__":
    ...
    create_video(RUTA, fps, f"monito_{caso}")
    remove_png(RUTA)