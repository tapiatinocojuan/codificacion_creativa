...
from paleta_colores import lluvia
...
def setup():
    global field, particulas, factor, s1, punto
    py5.size(720, 1080)
    field = VectorialField(
        py5.width, py5.height, 360, 0, caso, -invertir
    )
    s1, rutas = get_point_from_svg("DATA/Transformaciones_kawai_sombrilla.svg", 0.01, py5.width, layer="layer2")

    for ruta in rutas:
        print("ejecutando x ruta")
        for i, pts in enumerate(ruta[:-1]):
    ...
    for i in range(n):
        particulas.append(
            Particle(py5.random(0, py5.width), py5.random(0, py5.height/2))
        )
    py5.background(0)
...
class Particle():
    ...
    def __init__(self, x, y):
        self.x = x 
        self.y = y
        self.vx = 0 #Pixeles por frame
        self.vy = py5.random(100, py5.height) #Pixeles por frame
        self.color = py5.random_choice(lluvia)
    ...
    def draw(self):
        ...
        g =  self.color[1]*255
        b =  self.color[2]*255
        alpha =  py5.remap(py5.frame_count, 1, age_max, 120, 255)
    ...
    def update(self):
        ...
        if (self.x < 0 
            ...
            self.y = py5.random(0, py5.height/2)
    ...
    def calculate_field(self):
        ...
        for y in range(self.height):
            ...
            for x in range(self.width):
                ...
                if self.caso == 6:
                    aux[x] = (delta_x*self.invertir, delta_y*self.invertir)
                else:
                    if distancia == 0:
                        aux[x] = (0, 0)
                    else:
...
if __name__ == "__main__":
    ...
    create_video(RUTA, fps, f"corgi_sombrilla_particulas_{caso}")
    remove_png(RUTA)